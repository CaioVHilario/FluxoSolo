...
def popup_data_confirmation(df_new):
    ...
    if qtd_exists > 0:
        st.warning(
            f"Atenção, já extiste {qtd_exists} transações para este periodo de datas com este banco no seu banco de dados"
        )
        st.write(
            "Se você continuar podera duplicar dados. Deseja prosseguir mesmo assim?"
        )
