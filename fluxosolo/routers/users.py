...
from fluxosolo.core.secrets import get_current_user, get_password_hash
from fluxosolo.models.transactions import User
from fluxosolo.schemas import Message, UserPublic, UserSchema, UserUpdate
...
def update_user(
    ...
    user_db = session.scalar(select(User).where(User.id == current_user.id))
